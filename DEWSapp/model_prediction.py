...
historical_data_pathname = settings.BASE_DIR / "DEWSapp" / "dewsdocs" / "Shiroro-Spei.csv"


class ModelPrediction:
    @staticmethod
    def determine_forecast_steps(historical_data, current_year):
        latest_year_historical_data = pd.DatetimeIndex(historical_data['Date']).year.max()
    ...
    def forecast_spei(year):
        ...
        forecast_steps = ModelPrediction.determine_forecast_steps(historical_spei, year) # Adjust as needed

        forecast_index = pd.date_range(forecast_start_date, periods=forecast_steps, freq='M')
        forecast = model.get_forecast(steps=forecast_steps)
        forecast_values = forecast.predicted_mean
        
        forecast_data = pd.DataFrame({'Date': forecast_index, 'Forecasted_SPEI': forecast_values})
    ...
    def extract_spei(forecast_spei: pd.DataFrame, month: int, year: int):
        ...
        forecast_year = forecast_spei[pd.DatetimeIndex(forecast_spei['Date']).year == year]
        required_forecast = forecast_year[pd.DatetimeIndex(forecast_year['Date']).month == month]
